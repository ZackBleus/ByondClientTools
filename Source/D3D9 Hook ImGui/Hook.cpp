...
tPresent Hook::oPresent = nullptr; // Pointer of the original EndScene function
tReset Hook::oReset = nullptr; // Pointer of the original Reset function
tRelease Hook::oRelease = nullptr; // Pointer of the original Reset function
tDrawPrimitive Hook::oDrawPrimitive = nullptr;
tCreateTexture Hook::oCreateTexture = nullptr;
...
void Hook::HookDirectX()
{
	if (GetD3D9Device(d3d9Device, sizeof(d3d9Device)))
	{
		...
		oRelease = (tRelease)d3d9Device[2];
		oPresent = (tPresent)d3d9Device[17];
		oDrawPrimitive = (tDrawPrimitive)d3d9Device[81];
		oCreateTexture = (tCreateTexture)d3d9Device[23];
		DetourTransactionBegin();
		DetourUpdateThread(GetCurrentThread());
		DetourAttach(&(PVOID&)oEndScene, Drawing::hkEndScene);
		DetourAttach(&(PVOID&)oRelease, Drawing::hkRelease);
		DetourAttach(&(PVOID&)oPresent, Drawing::hkPresent);
		DetourAttach(&(PVOID&)oDrawPrimitive, Drawing::hkDrawPrimitive);
		DetourAttach(&(PVOID&)oCreateTexture, Drawing::hkCreateTexture);
...
void Hook::UnHookDirectX()
{
	...
	DetourDetach(&(PVOID&)oRelease, Drawing::hkRelease);
	DetourDetach(&(PVOID&)oPresent, Drawing::hkPresent);
	DetourDetach(&(PVOID&)oDrawPrimitive, Drawing::hkDrawPrimitive);
	DetourDetach(&(PVOID&)oCreateTexture, Drawing::hkCreateTexture);
